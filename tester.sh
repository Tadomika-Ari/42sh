...
if [ -d "$TESTS_FILE" ]
then
  if [ -f "$TESTS_FILE/tests" ]
  then
    TESTS_FILE="$TESTS_FILE/tests"
  elif [ -f "$TESTS_FILE/tests.sample" ]
  then
    TESTS_FILE="$TESTS_FILE/tests.sample"
  fi
fi
...
if [ ! -f "$TESTS_FILE" ]
then
  echo "No tests file. Expected 'tests' or 'tests/tests[.sample]'. Please read README.ME" >&2
  exit 1
fi
